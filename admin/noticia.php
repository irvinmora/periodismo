...
if (!$check_table || $check_table->num_rows == 0) {
    ...
    die("<h2 style='color: #d32f2f; text-align: center; margin-top: 50px;'><i class='fas fa-warning'></i> Base de Datos No Configurada</h2><p style='text-align: center;'><a href='migracion_multimedia.php'>Ejecutar Migración</a></p>");
}
...
<head>
    ...
    <title>Noticia Completa - Periodismo UTB</title>
...
<body>
    ...
    <div class="noticia-completa-container">
        ...
        if (!$noticia) {
            ...
            if (!empty($noticia['video_principal']) || !empty($noticia['audio_principal']) || !empty($noticia['link_principal'])) {
                ...
                if (!empty($noticia['audio_principal']) && file_exists('admin/audios/uploads/' . $noticia['audio_principal'])) {
                    ...
                    echo "<audio controls style='width: 100%; max-width: 400px;'>";
            ...
            if (count($subtitulos) > 0) {
                ...
                foreach ($subtitulos as $idx => $subtitulo) {
                    ...
                    echo "<h3>" . htmlspecialchars($subtitulo['subtitulo']) . "</h3>";
                    echo "</div>";
        ...
        ?>
                ...
                if ($result_subtitulos && $result_subtitulos->num_rows > 0) {
                    echo "<div class='subtitulos-seccion'>";
                    echo "<h2 class='subtitulos-titulo'><i class='fas fa-list-ol'></i> Secciones Relacionadas</h2>";
                    
                    while ($subtitulo = $result_subtitulos->fetch_assoc()) {
                        echo "<div class='subtitulo-card'>";
                        
                        // Header
                        echo "<div class='subtitulo-header'>";
                        echo "<span class='subtitulo-numero'>#" . $subtitulo['numero_subtitulo'] . "</span>";
                        echo "<h3>" . htmlspecialchars($subtitulo['subtitulo']) . "</h3>";
                        echo "</div>";
                ...
                echo "<a href='secciones.php' class='volver-btn'><i class='fas fa-arrow-left'></i> Volver a Secciones</a>";
                
            } else {
                echo "<div style='text-align: center; padding: 40px; background: #f9f9f9; border-radius: 8px;'>";
                echo "<i class='fas fa-exclamation-circle' style='font-size: 3rem; color: #999;'></i>";
                echo "<h2 style='color: #666; margin-top: 20px;'>Noticia no encontrada</h2>";
                echo "<a href='index.php' class='volver-btn' style='margin-top: 20px;'><i class='fas fa-home'></i> Ir al inicio</a>";
                echo "</div>";
            }
            ...
            echo "<div style='text-align: center; padding: 40px; background: #f9f9f9; border-radius: 8px;'>";
            echo "<i class='fas fa-exclamation-circle' style='font-size: 3rem; color: #999;'></i>";
            echo "<h2 style='color: #666; margin-top: 20px;'>Noticia no válida</h2>";
            echo "<a href='index.php' class='volver-btn' style='margin-top: 20px;'><i class='fas fa-home'></i> Ir al inicio</a>";
            echo "</div>";
        }
